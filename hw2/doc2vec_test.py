...
if __name__ == '__main__':
    ...
    model = Doc2Vec(size=300, alpha=0.025, min_alpha=0.025, workers=8, window=1, min_count=0)  # use fixed learning rate
    ...
    for h1, h2, ref in islice(sentences(), opts.num_sentences):
        ...
        print(-1 if cos_similar(ref,h1) > cos_similar(ref,h2) else # \begin{cases}
                (0 if cos_similar(ref,h1) == cos_similar(ref,h2)
                    else 1)) # \end{cases}
