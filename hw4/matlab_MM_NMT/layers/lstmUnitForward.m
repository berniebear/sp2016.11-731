function [lstmState] = lstmUnitForward(W, x_t, h_t_1, c_t_1, params, rnnFlags, dropoutmask_parallel_lstm)
% LSTM unit
% Input:
%   W: parameter
%   x_t: current input
...
% Thang Luong @ 2014, 2015, <lmthang@stanford.edu>
  ...
  if params.dropout<1 && rnnFlags.test==0
    if ~params.isGradCheck
        if isempty(dropoutmask_parallel_lstm) == 0
            dropoutMask = dropoutmask_parallel_lstm;
        else
