function [trainBatches, numTrainSents, numBatches, srcTrainFc7, srcTrainSents, tgtTrainSents] = trainLSTM(trainPrefix,validPrefix,testPrefix,srcLang,tgtLang,srcVocabFile,tgtVocabFile,outDir,varargin)  
% Multi-Modal Train Long-Short Term Memory (LSTM) models.
% Arguments:
...
%
% Po-Yao Huang, Fred Liu, add for parallel LSTM for multiple modality MT
% task
...
% Hieu Pham: beam-search decoder.
  ...
  %params =parameter_parser(trainPrefix,validPrefix,testPrefix,srcLang,tgtLang,srcVocabFile,tgtVocabFile,outDir,varargin);
%% Argument Parser
  ...
  addOptional(p,'numEpoches', 8, @isnumeric); % num epoches
  ...
  [model, params] = initLoadModel(params); % remark : inite multiple lstm
  ...
  if params.isBi
    ...
    params.srcTrainImgId = fopen('train_images_rcnn.txt', 'r'); % need to be train, coding with test
  else
    fprintf(2, '# Load train data tgtFile "%s"\n', params.tgtTrainFile);
  end
  params.tgtTrainId = fopen(params.tgtTrainFile, 'r');
  [trainBatches, numTrainSents, numBatches, srcTrainFc7, srcTrainSents, tgtTrainSents] = loadTrainBatches(params);
  % print
  if params.isBi
  ...
  while(isRun)
    ...
    for batchId = 1 : numBatches
      ...
      [gradNorm_W_img, ~] = computeGradNorm(grad, params.batchSize, {'W_img'}); % historical reason: we exclude W_emb % indNorms
      ...
      [params, startTime] = postTrainIter(model, costs, gradNorm, gradNorm_W_img, trainData, validData, testData, params, startTime, srcTrainFc7, srcTrainSents, tgtTrainSents);
    end % end for batchId
    ...
    %% end of an epoch
    if numTrainSents == 0 
      [trainBatches, numTrainSents, numBatches, srcTrainFc7, srcTrainSents, tgtTrainSents, startTime, params] = postTrainEpoch(model, validData, testData, startTime, params);
...
function [model] = initLSTM(params)
  ...
  % W_src
  if params.isBi
    model.W_src = cell(params.numLayers, 1);    % create a model stuct
  ...
  % W_img
  if params.isBi
    model.W_img = initMatrixRange(params.initRange, [params.lstmSize, 4096], params.isGPU, params.dataType);
    model.b_img = initMatrixRange(params.initRange, [params.lstmSize, 1], params.isGPU, params.dataType);
  end
  ...
  if params.attnFunc>0 % attention mechanism
    ...
    % attn_t = H_src * a_t , contxt vector (c_t)
    % h_attn_t = f(W_h * [attn_t; h_t]) => 2
    model.W_h = initMatrixRange(params.initRange, [params.lstmSize, 2*params.lstmSize], params.isGPU, params.dataType); % W_c in paper
  end
...
function [params, startTime] = postTrainIter(model, costs, gradNorm,gradNorm_W_img, trainData, validData, testData, params, startTime, srcTrainFc7, srcTrainSents, tgtTrainSents)
  %% log info
  ...
  if mod(params.iter, params.logFreq) == 0
    ...
    logStr = sprintf('%d, %d, %.2fK, %g, %.2f, gN=%.2f, g_img=%.2f,%s', params.epoch, params.iter, params.speed, params.lr, ...
        params.scaleTrainCosts.total, gradNorm, gradNorm_W_img ,datestr(now));
    ...
    %model.W_img_emb = initMatrixRange(params.initRange, [params.lstmSize, 4096], params.isGPU, params.dataType); 
  %% eval
  if mod(params.iter, params.evalFreq) == 0    
    % eval, save, and decode
    [params] = evalSaveDecode(model, validData, testData, params, srcTrainFc7, srcTrainSents, tgtTrainSents);
...
function [trainBatches, numTrainSents, numBatches, srcTrainFc7, srcTrainSents, tgtTrainSents, startTime, params] = postTrainEpoch(model, validData, testData, startTime, params)
  % seek to the beginning
  if params.isBi
    fseek(params.srcTrainId, 0, 'bof');
    fseek(params.srcTrainImgId, 0, 'bof');
  end
...
function [params] = evalSaveDecode(model, validData, testData, params, srcTrainFc7, srcTrainSents, tgtTrainSents)
  % eval
  ...
  if params.isBi && params.decode==1
    ...
    size(srcTrainSents(1));
    size(tgtTrainSents(1));
    srcTrainSents(1);
    tgtTrainSents(1);
    size(srcTrainFc7(:,1));
    decodeSent(srcTrainFc7(:,1,:) ,srcTrainSents(1), tgtTrainSents(1), model, params);
    decodeSent(validData.srcFc7(:,validId,:), validData.srcSents(validId), validData.tgtSents(validId), model, params);
    decodeSent(testData.srcFc7(:,testId,:), testData.srcSents(testId), testData.tgtSents(testId), model, params);
  end
end

function decodeSent(srcFc7, srcSent, tgtSent, model, params)
  params.preeosId = -1;
  [decodeData] = prepareData(srcFc7, srcSent, tgtSent, 1, params);
...
function [trainBatches, numTrainSents, numBatches, srcTrainFc7, srcTrainSents, tgtTrainSents] = loadTrainBatches(params)
  % load src
  if params.isBi
    [srcTrainFc7, srcTrainSents, ~, srcTrainLens] = loadBatchData_img_mm(params.srcTrainImgId, params.srcTrainId, params.baseIndex, params.chunkSize);
  else
    srcTrainSents = {};
  end
  % load tgt
  [tgtTrainSents, numTrainSents, tgtTrainLens] = loadBatchData(params.tgtTrainId, params.baseIndex, params.chunkSize);% no img for tgt
  % sorting
  if params.sortBatch
    ...
    if params.isBi
      ...
      srcTrainFc7 = srcTrainFc7(sortIndices);
      srcTrainFc7 = wrap_fc7(srcTrainFc7);
    end
  end
  % split into batches
  if numTrainSents>0
    ...
    for batchId = 1 : numBatches
      ...
      if params.isBi
        ...
        srcBatchFc7 = srcTrainFc7(:,startId:endId,:);
      end
      tgtBatchSents = tgtTrainSents(startId:endId);
      tgtBatchLens = tgtTrainLens(startId:endId);
      trainBatches{batchId} = prepareData(srcBatchFc7, srcBatchSents, tgtBatchSents, 0, params, srcBatchLens, tgtBatchLens);
    end
...
function [data] = loadPrepareData(params, prefix, srcVocab, tgtVocab)
  [srcFc7, srcSents, tgtSents, numSents] = loadBiData_mm(params, prefix, srcVocab, tgtVocab);
  
  [data] = prepareData_mm(srcFc7, srcSents, tgtSents, 1, params); % isTest = 1
  %data.srcFc7 =zeros(4096, length(tgtSents),3);
  %data.srcFc7 =warp_fc7(data.srcFc7);
  fprintf(2, '  numSents=%d, numWords=%d\n', numSents, data.numWords); %numWords = sum(tgtMask(:))
